...
- **El servidor no puede validar la sesión.** La sesión vive en el navegador, no en una cookie que el servidor pueda comprobar. Desde el 2026-09-15, la app pone una cookie que solo avisa que hay sesión, y Vercel redirige al login a quien llega a `app.html` sin ella, sin entregar la página. Pero esa cookie no es la sesión y se podría falsificar: quien lo haga recibe la página vacía, que sin sesión no muestra nada y manda al login, y la base no le entrega ningún dato. La protección real sigue siendo el RLS.
